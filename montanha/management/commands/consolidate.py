...
from montanha.models import Institution, Expense
from montanha.models import ExpenseNature, Legislator, Supplier
from montanha.models import PerNature, PerNatureByYear, PerNatureByMonth
from montanha.models import PerLegislator, BiggestSupplierForYear
from montanha.util import filter_for_institution
from montanha.util import get_date_ranges_from_data, ensure_years_in_range
        ...
        for institution in institutions:
            ...
            for item in data:
                ...
                for legislature in institution.legislature_set.all():
                    pargs = (
                        institution.siglum, nature.name,
                        legislature.date_start.year, legislature.date_end.year)
                    p1 = 'Consolidating nature'
                    p2 = 'totals for legislature'
                    print u'[%s] ' + p1 + ' %s ' + p2 + ' %d-%d…' % pargs
                ...
                # By Year
                for year in years:
                    print u'[%s] Consolidating nature %s total for year %d' % (
                    ...
                    for month in range(1, 13):
                        print u'[%s] Consolidate nature %s total for %d-%d' % (
